...
public class AppStoreOrderServiceImpl extends ServiceImpl<StoreOrderMapper,StoreOrderDO> implements AppStoreOrderService {
    ...
    public Map<String, Object> createOrder(Long uid,  AppOrderParam param) {
        ...
        // 使用OrderStockService检查库存
        orderStockService.checkStock(uid, productIds, numbers, specs);

        // 计算商品数量
        for (String number : numbers) {
            totalNum += Integer.valueOf(number);
        }

        // 使用OrderCalculationService计算商品总价
        BigDecimal sumPrice = orderCalculationService.calculateProductTotal(productIds, numbers, specs);
        ...
        // 使用OrderCalculationService计算最终支付价格
        BigDecimal payPrice = orderCalculationService.calculatePayPrice(sumPrice, postagePrice,
                couponPrice, deductionPrice, param.getOrderType());

        // 使用OrderCalculationService计算奖励积分
        BigDecimal gainIntegral = orderCalculationService.calculateGainIntegral(productIds);
...
//        }
        ...
        //使用OrderReversalService奖励积分
        orderReversalService.grantUserIntegral(order);
